#Instructions:
#1. Create a CSV with 4 columns in the following order: full_table_id, field_name, tag key, tag value
#	full_table_is must be lowercase in the format database:schema.table, field_name must also be lowercase
	...
	with open(csvFileName,"r") as tags_to_import:
		...
		for row in tags:
			...
			if mconDict[row[0]]:
				...
				temp_obj=dict(mconId=str(mconDict[row[0]]+ '+++' + row[1].lower()).replace('++table++','++field++'),propertyName=row[2],propertyValue=row[3])
				print(temp_obj)
